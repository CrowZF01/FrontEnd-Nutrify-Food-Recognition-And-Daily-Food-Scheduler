...
class ApiService {
  ...
  /// Upload foto untuk deteksi ingredient dan dapatkan gambar hasil deteksi
  static Future<Uint8List> uploadPhotoForDetection({
    ...
    request.files.add(
      http.MultipartFile.fromBytes(
        ...
        imageBytes,
        filename: filename,
        contentType: MediaType('image', 'jpg'), // ✅ contentType: image/jpg
      ),
    );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Gagal mendeteksi bahan makanan (Status ${response.statusCode})');
      }
    } catch (e) {
      throw Exception('Kesalahan upload foto: $e');
    }
  }
}
